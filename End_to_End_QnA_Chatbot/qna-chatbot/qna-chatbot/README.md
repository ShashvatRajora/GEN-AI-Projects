...
Your repo needs two top-level folders: `frontend/` and `backend/`.
 
```bash
git init
git add .
git commit -m "initial commit"
git remote add origin https://github.com/YOUR_USERNAME/qna-chatbot.git
...
6. Click **Deploy** — Railway gives you a public URL like `https://qna-chatbot-backend.up.railway.app`
> **Alternative: Render**
> Same steps at https://render.com → New Web Service → connect repo → set Root Directory to `backend` → add the same env vars.
...
| Variable | Value |
|----------|-------|
| `REACT_APP_API_URL` | Your Railway/Render backend URL (from Step 2) |
 
6. Click **Deploy** — Vercel gives you a URL like `https://qna-chatbot.vercel.app`
---
 
### Step 4 — Update CORS on the backend
 
Go back to Railway/Render → Variables → update:
 
```
ALLOWED_ORIGINS=https://qna-chatbot.vercel.app
```
 
Redeploy the backend. Done — your app is live end-to-end.
